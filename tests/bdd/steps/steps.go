...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.When(`^Claude invokes the onboard_user tool with skills "([^"]*)" but no resume$`, s.invokeOnboardUserSkillsButNoResume)
	ctx.When(`^Claude invokes the onboard_user tool with accomplishments text but no resume$`, s.invokeOnboardUserAccomplishmentsButNoResume)
	ctx.When(`^Claude invokes the onboard_user tool with resume_content "([^"]*)" and resume_label "([^"]*)" but no skills$`, s.invokeOnboardUserResumeNoSkills)
	ctx.When(`^Claude invokes the onboard_user tool with resume_content "([^"]*)" and resume_label "([^"]*)" but no accomplishments$`, s.invokeOnboardUserResumeNoAccomplishments)
	...
	ctx.When(`^the user runs: go-apply onboard --skills (\S+)$`, s.cliOnboardSkillsOnly)
	ctx.When(`^the user runs: go-apply onboard --accomplishments (\S+)$`, s.cliOnboardAccomplishmentsOnly)
	...
	ctx.Then(`^the response includes a warning that T(\d+) tailoring is unavailable without skills$`, s.assertWarningT1Unavailable)
	ctx.Then(`^the response includes a warning that T(\d+) tailoring is unavailable without accomplishments$`, s.assertWarningT2Unavailable)
	ctx.Then(`^go-apply returns an error indicating the user must run go-apply onboard first$`, s.assertNeedToOnboard)
	...
	ctx.Given(`^the user profile contains a resume, skills, and accomplishments$`, s.profileWithResumeSkillsAccomplishments)
	ctx.Given(`^the user profile contains a resume only \(no skills, no accomplishments\)$`, s.profileWithResumeOnly)
	ctx.Given(`^the profile contains skills and accomplishments$`, s.addSkillsAndAccomplishments)
	...
	ctx.When(`^the user supplies a job description via (URL|raw text)$`, s.userSuppliesJobDescriptionVia)
	ctx.When(`^the user supplies a job description with channel "([^"]*)"$`, s.userSuppliesTextWithChannel)
	ctx.When(`^the best resume score is >= (\d+)$`, s.assertScoreMeetsThreshold)
	...
	ctx.Then(`^go-apply extracts required and preferred keywords$`, s.assertKeywordsExtracted)
	...
	ctx.Then(`^T(\d+) keyword injection runs against the Skills section$`, s.assertT1Ran)
	ctx.Then(`^T(\d+) bullet rewrites run using the accomplishments$`, s.assertT2Ran)
	ctx.Then(`^if the final score is >= (\d+), a cover letter is generated for the best-matching resume$`, s.assertCoverLetterIfScore)
	ctx.Then(`^the result contains the base score, tailored score, extracted keywords, and cover letter if generated$`, s.assertFullTailoredResult)
	ctx.Then(`^the tailoring step is skipped$`, s.assertTailoringSkipped)
	ctx.Then(`^if the base score is >= (\d+), a cover letter is generated$`, s.assertCoverLetterIfBaseScore)
	ctx.Then(`^the result contains the base score, extracted keywords, and cover letter if generated$`, s.assertBaseScoreResult)
	ctx.Then(`^go-apply skips the tailoring step$`, s.assertTailoringSkipped)
	ctx.Then(`^the result includes an advisory: "([^"]*)"$`, s.assertAdvisoryMessage)
	ctx.Then(`^if the best score is >= (\d+), T(\d+) keyword injection runs on the best-matching resume$`, s.assertT1OnBest)
	ctx.Then(`^if the best score is >= (\d+), T(\d+) bullet rewrites run on the best-matching resume$`, s.assertT2OnBest)
	ctx.Then(`^a cover letter is generated if the final score is >= (\d+)$`, s.assertCoverLetterFinalScore)
	ctx.Then(`^go-apply returns an error indicating the pipeline cannot run without the orchestrator$`, s.assertPipelineInoperable)
	ctx.Then(`^no scores or cover letter are returned$`, s.assertNoScoresOrCoverLetter)
	ctx.Then(`^the orchestrator performs keyword matching to retrieve relevant profile chunks$`, s.assertOrchestratorKeywordMatching)
