...
func (r *ResumeRepository) sidecarPath(label string) string {
	return filepath.Join(r.inputsDir, label+".sections.json")
}

// LoadSections reads the sidecar JSON file for the given resume label.
...
func (r *ResumeRepository) LoadSections(label string) (model.SectionMap, error) {
	...
	if err != nil {
		...
		return model.SectionMap{}, fmt.Errorf("read sidecar %s: %w", path, err)
	}
	var sm model.SectionMap
	if err := json.Unmarshal(data, &sm); err != nil {
		return model.SectionMap{}, fmt.Errorf("unmarshal sidecar %s: %w", path, err)
	}
	...
	if err := model.ValidateSectionMap(&sm); err != nil {
		return model.SectionMap{}, fmt.Errorf("invalid sidecar %s: %w", path, err)
	}
	return sm, nil
}

// SaveSections marshals sm to JSON and writes it atomically to the sidecar for label.
